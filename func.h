...
double difazio(double a, double b, double target)
{
    ...
    if(cap>0)
    {
        ...
        if(abs((target-fm))<PRECISION)   {std::cout<<"Trovato valore esatto f(x)="<<fm<<" per x= "<<m<<std::endl; return fm;}                                //se è uguale al target siamo fortunati, non c'è bisogno di procedere
